...
static void FindCustomer(SQLiteConnection myConnection)
{
        ...
        command = myConnection.CreateCommand();
        command.CommandText = $"SELECT customerId, customer.FirstName, customer.LastName, status.statustype " +
        $"FROM customerStatus " +
        $"JOIN customer ON customerId = customerStatus.customerId " +
        $"JOIN status ON status.rowid = customerStatus. " +
